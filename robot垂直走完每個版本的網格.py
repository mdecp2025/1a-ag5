...
w = World(15, 10)    # 建立 10x10 的世界
w.robot(1, 1)        # 在 (1,1) 放置一台 robot

r = AnimatedRobot(w, 1, 1)


def walk_all_grid_vertical(robot, world):
    """
    讓機器人垂直蛇形繞行整個 grid。
    robot: AnimatedRobot 物件
    world: World 物件
    """
    # 讓機器人一開始朝北
    robot.turn_left()  # 由原本的東轉成北

    for col in range(world.width):
        # 垂直走完整列
        robot.move(world.height - 1)
